<script>
	...
	let anglestops = [0, 30, 60, 90, 120, 150].map(
		(x) => (x * Math.PI) / 180.0
	);
	export let kind;
</script>
...
{:else if kind === "polar"}
	...
	{#each anglestops as angle}
		<!-- lines originating from center of the circle -->
		<line
			x1={-5000 * Math.cos(angle)}
			y1={-5000 * Math.sin(angle)}
			x2={5000 * Math.cos(angle)}
			y2={5000 * Math.sin(angle)}
		/>
	{/each}
{/if}
